...
class TestContainerHandler(TestCase):
    def setUp(self) -> None:
        self.container_handler = ContainerHandler(
            language="node", container_name="NAME"
        )
...
class TestFilesHandler(TestCase):
    ...
    @patch("os.system")
    def test_should_call_correct_copy_result_from_container_command(
        self, mock_os
    ) -> None:
...
class TestExecuteTest(TestCase):
    def setUp(self) -> None:
        self.execute_test = ExecuteTest(
            input="hello", output="world", code='console.log("hello")', language="node"
        )
