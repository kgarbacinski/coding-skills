...
class ContainerHandler:
    ...
    def __init__(self, language: str, container_name: str) -> None:
        ...
        self.__file_extension = self.__language_specs.language_extensions.get(
            self.__language
        )
    ...
    def stop_container(self) -> None:
        stop_container = (
            f"docker stop {self.__container_name} && docker rm {self.__container_name}"
        )
...
class FilesHandler:
    ...
    def __init__(
        self, input: str, output: str, code: str, language: str, container_name: str
    ) -> None:
        self.__input = input
        ...
        self.__code_validator = open(
            self.__language_specs.code_validators.get(self.__language)
        ).read()
        self.__xml_template = open(self.__language_specs.xml_template).read()
        self.__exec_folder_path = os.path.join(os.getcwd(), f"checker/temp/exec_files")
        self.__result_folder_path = os.path.join(
            os.getcwd(), f"checker/temp/result_files"
        )
        self.__container_name = container_name
    ...
    def __generate_testing_values_file(self) -> None:
        values = self.__xml_template.replace("INPUT_VALUE", self.__input).replace(
