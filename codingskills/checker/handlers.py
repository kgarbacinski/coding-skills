...
class ContainerHandler:
    ...
    Methods: 
        run_container(): runs docker container with predefined image env (based on the language chosen).
...
class FilesHandler:
    """
    This class takes the input from frontend, generates / moves files with code to be executed remotly.
    ...
    Methods: 
        generate_code_validation(): reads validation code from /configs/ and saves it to the file with extension based on user decision (.java, .py, etc).
        generate_submitted_code_file(): takes the input from user in frontend and saves it to the files with chosen language extension.
        ...
        get_result_value(): reads the calue from result.txt
